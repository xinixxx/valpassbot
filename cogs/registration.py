...
class PlayerInfoModal(discord.ui.Modal, title="내전 참여 정보 등록"):
    ...
    chzzk_nickname = discord.ui.TextInput(label="치지직 닉네임 (선택)", placeholder="사용하는 치지직 닉네임을 입력하세요.", required=False)
    highest_tier = discord.ui.TextInput(label="최고 티어 (선택)", placeholder="예시) 다이아몬드 1", required=False)
    current_tier = discord.ui.TextInput(label="현재 티어 (선택)", placeholder="예시) 플래티넘 3", required=False)
